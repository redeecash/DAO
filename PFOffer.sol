...
contract PFOffer {
    ...
    function sign() {
        (uint256 _, , , uint256 votingDeadline, , ) = client.proposals(proposalID);
    ...
    function withdraw() noEther {
        ...
        uint256 lastWithdrawalReset = lastWithdrawal;
        lastWithdrawal = now;
    ...
    function watchProposal(uint _proposalID) noEther onlyContractor {
        (address recipient, , , uint256 votingDeadline, bool open, ) = client.proposals(_proposalID);
    ...
    function checkVoteStatus() noEther {
        ( , , , uint256 votingDeadline, , , , , , uint256 yea, uint256 nay, ) = client.proposals(proposalID);
    ...
    receive () external payable {
        revert("this is a business contract, no donations"); 
    }
}
